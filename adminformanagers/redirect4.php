...
?>
<?php 
 $articless = mysqli_query($connection, "SELECT * FROM alternators WHERE id = " . (int) $_GET['id']);
...
<?php
  ...
  //  imagedestroy($scaled);
    ...
    if (isset($_POST['upload2212'])) {
    $target = "../img/imgProducts/".basename( time() . $_FILES['image']['name']);
    $imageslideInt = time() . $_FILES['image']['name'];
    $sql = "INSERT INTO alternatorsphoto (imgsrc, blickhref) VALUES ('/img/imgProducts/$imageslideInt', '".$artt['linkhref']."' )";
  ...
  var_dump($sql);
  $result = mysqli_query($connection, "SELECT * FROM imginterier");
