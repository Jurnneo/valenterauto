...
<head>
    ...
    <meta name="description" content="<?php $alternatorg = $row131['voltag']; $query = "SELECT * FROM where_is_use_table WHERE linkalternator =  '$alternatorg'"; $statement = $connect->prepare($query); $statement->execute();
...
<div class="sidebaropt">
  ...
  <div class="link">
    <a href="../electromotor"><div class="sidebartext">ELECTROMOTOARE</div></a>
  </div>
  <div class="link">
    <a href="index.php"><div class="sidebartext">ALTERNATOARE</div></a>
  </div>
  <div class="link">
...
<?php
  $categorylink = mb_strtolower($row131["category"]);
...
<div class="footerrtyInside11erCatalog">
  ...
  <a href="categories/<?php echo $categoryreplaced ?>/index.php"><p><img src="/img/imgSite/catalogButton.png" class="footerrtyInside11erCatalogImg"><?php echo mb_strtoupper($row131['category']); ?></p></a>
  <p class="catalogButtons7">/</p>
...
$link = $row131['link'];
$query = "SELECT alternators.id, alternators.imgsrc, alternators.link, alternators.category, alternators.voltag, alternators.product_status FROM `alternators` WHERE category = '$category' AND product_status = '1' AND link != '$link' AND voltag LIKE '%".$voltajj."%'";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
...
<div class="tablesArt">
 ...
 <div class="tablesArt1ProductCrosses">
  ...
  <?php
   $alternatorv = $row131['voltag'];
   $query = "SELECT * FROM krosstable WHERE linkalternator =  '$alternatorv'";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
   foreach ($result as $row1311) {
