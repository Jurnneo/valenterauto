...
<head>
    <title>
       <?php
         $query = "SELECT * FROM fuliecuambrenajinitial WHERE id = " . (int)$_GET['id'];
    ...
    <meta name="keywords" content="<?php $alternatorv = $row131['voltag']; $query = "SELECT * FROM fuliecuambrenajkrosstable WHERE linkalternator = '$alternatorv'"; $statement = $connect->prepare($query); $statement->execute(); $result = $statement->fetchAll(); foreach ($result as $rowkeywords) { ?><?php if ( $row131['voltag'] != NULL) echo 'VOLTAG - '.$row131['voltag'].',' ?> <?php if ( $rowkeywords['cas'] != NULL) echo 'CAS - '.$rowkeywords['cas'].','
 ...
 <div class="articlePhoto">
  <div class="productArtImg">
   <div class="productArtImgSmall">
    ...
    <img src="../../img/imgfuliecuambreiaj/<?php echo $row131['voltag']; ?>_1.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">
    <img src="../../img/imgfuliecuambreiaj/<?php echo $row131['voltag']; ?>_2.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">   
    <img src="../../img/imgfuliecuambreiaj/schemafuliecuambreiaj.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">  

   </div>
   <div class="productArtImgContainerBig">
    <img id="imageBoxBig" src="../../img/imgfuliecuambreiaj/<?php echo $row131['voltag']; ?>_1.jpg">
   </div>               
  </div>
 </div>
...
<?php
   ...
   $query = "SELECT * FROM fuliecuambrenajkrosstablefilter WHERE linkalternator = '$alternatorv'";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
...
<div class="aplicaridiod tablesArt1ProductCrossesonthesite">
 ...
 $link = $row131['link'];
 $query = "SELECT id, imgsrc, link, category, title, voltag, product_status FROM `fuliecuambrenajinitial` WHERE category = '$category' AND product_status = '1' AND link != '$link' AND voltag = '".$voltajj."' ORDER BY id DESC";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 ...
 $totalsum = $rowpsklad['Column_5sum'];
  ...
  <a href="article.php?id='. $row['id'] .'"><li><img class="nodatablock datanotfoundimg1" src="../../img/imgfuliecuambreiaj/'.$row['voltag'].'_1.jpg" alt="0">'.$row['link'].''.$finalinf.'</li></a>
...
<div class="tablesArt">
 ...
 <div class="tablesArt1ProductCrosses">
  ...
  <?php
   $alternatorv = $row131['voltag'];
   $query = "SELECT * FROM fuliecuambrenajkrosstable WHERE '$alternatorv' = linkalternator";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
   foreach ($result as $row1311) {
