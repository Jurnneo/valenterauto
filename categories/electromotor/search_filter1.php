...
?>       
 ...
 <div class="CatalogGrid11">
  ...
  <a href="search_filter.php"><input style="font-family: 'Montserrat', sans-serif; margin-left: 5px; width: 117px; cursor: pointer;" class="buttonfilterlink" value="STERGE FILTRUL"></a>
 </div>
...
<div class="CatalogGrid2">
 <div class="contentCatalog">
  <div id="myULssh" class="row filter_data" >
    ...
    $count = mysqli_query($link, "SELECT COUNT(*) FROM electromotorinitial WHERE product_status = '1'");
  ...
  $result = mysqli_query($link, "SELECT electromotorinitial.id, electromotorinitial.link, electromotorinitial.brend, electromotorinitial.title, electromotorinitial.imgsrc, electromotorinitial.price, electromotorinitial.category, electromotorinitial.product_status, electromotorinitial.voltag, electromotorkrosstablefilter.linkalternator, krosstablefilter_categories.ch1, electromotorkrosstablefilter.ch1_2, krosstablefilter_categories.ch2, electromotorkrosstablefilter.ch2_2, krosstablefilter_categories.ch3, electromotorkrosstablefilter.ch3_2, krosstablefilter_categories.ch4, electromotorkrosstablefilter.ch4_2, krosstablefilter_categories.ch5, electromotorkrosstablefilter.ch5_2, krosstablefilter_categories.ch6, electromotorkrosstablefilter.ch6_2, krosstablefilter_categories.ch7, electromotorkrosstablefilter.ch7_2, krosstablefilter_categories.ch8, electromotorkrosstablefilter.ch8_2, krosstablefilter_categories.ch9, electromotorkrosstablefilter.ch9_2, krosstablefilter_categories.ch10, electromotorkrosstablefilter.ch10_2 FROM electromotorinitial
    ...
    WHERE product_status = '1' $query_ch1_2 $query_ch1_3 $query_ch1_4 ORDER BY id DESC $query_start_num");
 ...
 $totalsum = $rowpsklad['Column_5sum'];
  ...
  if (!empty($url)) {
    ...
    if($_GET["brend"] || $_GET["ch1_2"] || $_GET["ch2_2"]){
        $pagevar = '&page=';
    }else{
        $pagevar = '?page=';
    }
    ...
    $urlfin = $newString;
    $pagevar1 = $pagevar;
    $urlfin1 = '<a href="'.$urlfin.''.$pagevar1.'1">1</a>
    <a href="'.$urlfin.''.$pagevar1.'2">2</a>
    <a href="'.$urlfin.''.$pagevar1.'3">3</a>
    <a href="'.$urlfin.''.$pagevar1.'4">4</a>
    <a href="'.$urlfin.''.$pagevar1.'5">5</a>';
    }  

    echo $urlfin1;

    if ($page != 1) {$pstr_prev = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page - 1).'">&lt</a></li>';}

    if ($page != $total) $pstr_next = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page + 1).'">&gt</a></li>';


    if ($page - 5 > 0) $page5left = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page - 5).'">&gt'.($page - 5).'</li>';
    if ($page - 4 > 0) $page4left = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page - 4).'">&gt'.($page - 4).'</li>';
    if ($page - 3 > 0) $page3left = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page - 3).'">&gt'.($page - 3).'</li>';
    if ($page - 2 > 0) $page2left = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page - 2).'">&gt'.($page - 2).'</li>';
    if ($page - 1 > 0) $page1left = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page - 1).'">&gt'.($page - 1).'</li>';


    if ($page - 5 <= $total) $page5right = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page + 5).'">&gt'.($page + 5).'</li>';
    if ($page - 4 <= $total) $page4right = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page + 4).'">&gt'.($page + 4).'</li>';
    if ($page - 3 <= $total) $page3right = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page + 3).'">&gt'.($page + 3).'</li>';
    if ($page - 2 <= $total) $page2right = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page + 2).'">&gt'.($page + 2).'</li>';
    if ($page - 1 <= $total) $page1right = '<li><a class="pstr_prev" href="'.$urlfin.''.$pagevar1.''.($page + 1).'">&gt'.($page + 1).'</li>';

    if ($page + 5 < $total) {
      $strtotal = '<li><p class="nav-point">...</p></li><li><a href="'.$urlfin.''.$pagevar1.''.$total.'">'.$total.'</a></li>';
    }else{
      $strtotal = "";
    }
    ...
    if ($total > 1) {
      ...
      <ul>
      ';
      echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pstr-active' href="$urlfin.$pagevar1.$page">".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$pstr_prev;
      echo '
      </ul>
      </div>
      '
...
<footer style="margin-top: 10px;" class="footSmall">
 ...
 <div class="footSmall1">
  ...
  <p><?php $url;?> <span>Valenterauto</span> activează pe piața <span>Romaniei</span> din anul 2010.</p>
