...
<head>
    <title>
       <?php
         $query = "SELECT * FROM alternatorinitial WHERE id = " . (int)$_GET['id'];
    ...
    <meta name="keywords" content="<?php $alternatorv = $row131['voltag']; $query = "SELECT * FROM alternatorkrosstable WHERE linkalternator = '$alternatorv'"; $statement = $connect->prepare($query); $statement->execute(); $result = $statement->fetchAll(); foreach ($result as $rowkeywords) { ?><?php if ( $row131['voltag'] != NULL) echo 'VOLTAG - '.$row131['voltag'].',' ?> <?php if ( $rowkeywords['cas'] != NULL) echo 'CAS - '.$rowkeywords['cas'].','
 ...
 <div class="wow fadeIn articleText" data-wow-duration="2.5s">
  ...
  ?>
   ...
   $titleupakovka = $row131["title"];
        ...
        $query = "SELECT SUM(Column_4) AS Column_5sum FROM `alternatorstock` WHERE `Column_3` = '$titleupakovka' AND (`Column_2` = '$linksklad' OR `Column_1` = '$linksklad')";
        $statement = $connect->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll();
...
$link = $row131['link'];
$id = $row131['id'];
 $query = "SELECT id, imgsrc, link, title, category, voltag, product_status FROM `alternatorinitial` WHERE category = '$category' AND product_status = '1' AND id != '$id' AND voltag = '".$voltajj."' ORDER BY id DESC";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
