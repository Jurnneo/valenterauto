...
<head>
    <title>
       <?php
         $query = "SELECT * FROM electromotorinitial WHERE id = " . (int)$_GET['id'];
    ...
    <meta name="keywords" content="<?php $alternatorv = $row131['voltag']; $query = "SELECT * FROM electromotorkrosstable WHERE linkalternator = '$alternatorv'"; $statement = $connect->prepare($query); $statement->execute(); $result = $statement->fetchAll(); foreach ($result as $rowkeywords) { ?><?php if ( $row131['voltag'] != NULL) echo 'VOLTAG - '.$row131['voltag'].',' ?> <?php if ( $rowkeywords['cas'] != NULL) echo 'CAS - '.$rowkeywords['cas'].','
 ...
 <div class="articlePhoto">
  <div class="productArtImg">
   <div class="productArtImgSmall">
    ...
    <img src="/img/product/<?php echo $row131['voltag']; ?>-1.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">   

    <img src="/img/product/<?php echo $row131['voltag']; ?>-2.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">  

    <img src="/img/product/<?php echo $row131['voltag']; ?>-3.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">  

   </div>
   <div class="productArtImgContainerBig">
    <img id="imageBoxBig" src="/img/product/<?php echo $row131['voltag']; ?>-1.jpg">
   </div>               
  </div>
 </div>
 ...
 <div class="wow fadeIn articleText" data-wow-duration="2.5s">
  ...
  ?>
   ...
   $titleupakovka = $row131["title"];
        ...
        $query = "SELECT SUM(Column_4) AS Column_5sum FROM `electromotorstock` WHERE `Column_3` = '$titleupakovka' AND (`Column_2` = '$linksklad' OR `Column_1` = '$linksklad')";
        $statement = $connect->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll();
        ...
        if ($totalsum > 0){
          $finalinf = '<div class="stockondivart stockondiv"><p class="fruit stockon">IN STOC</p></div>';
        }
        ...
        if ($totalsum == 0){
          $finalinf = '<div class="stockondiv2art stockondiv2"><p class="fruit stockon2">LA COMANDA</p></div>';
        }

        if ($totalsum == NULL){
          $finalinf = '<div class="stockondiv2art stockondiv1"><p class="fruit stockon1">SE CONCRETIZEAZA</p></div>';
        }
    ?>
...
<?php
   ...
   $query = "SELECT * FROM electromotorkrosstablefilter WHERE linkalternator = '$alternatorv'";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
...
$link = $row131['link'];
$id = $row131['id'];
 $query = "SELECT id, imgsrc, link, title, category, voltag, product_status FROM `electromotorinitial` WHERE category = '$category' AND product_status = '1' AND id != '$id' AND voltag = '".$voltajj."' ORDER BY id DESC";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 ...
 $totalsum = $rowpsklad['Column_5sum'];
   ...
   <li class="pozitiisimilareanalogii fruit1 fruit">POZITII SIMILARE : </li>

   <a href="article.php?id='. $row['id'] .'"><li><img class="nodatablock datanotfoundimg1" src="/img/product/'.$row131['voltag'].'-1.jpg" alt="DATA NOT FOUND">'.$row['link'].''.$finalinf.'</li></a>
...
<div class="tablesArt">
 ...
 <div class="tablesArt1ProductCrosses">
  ...
  <?php
   $alternatorv = $row131['voltag'];
   $query = "SELECT * FROM electromotorkrosstable WHERE '$alternatorv' = linkalternator";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
   foreach ($result as $row1311) {
