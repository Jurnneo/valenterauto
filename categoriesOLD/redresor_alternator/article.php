...
<head>
    ...
    <meta name="keywords" content="<?php $alternatorv = $row131['voltag']; $query = "SELECT * FROM redresorkrosstable WHERE linkalternator = '$alternatorv'"; $statement = $connect->prepare($query); $statement->execute(); $result = $statement->fetchAll(); foreach ($result as $rowkeywords) { ?><?php if ( $row131['voltag'] != NULL) echo 'VOLTAG - '.$row131['voltag'].',' ?> <?php if ( $rowkeywords['cas'] != NULL) echo 'CAS - '.$rowkeywords['cas'].','
 ...
 <div class="articlePhoto">
  <div class="productArtImg">
   <div class="productArtImgSmall">
    ...
    <img src="../../img/imgredresor/<?php echo $row131['voltag']; ?>_1.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">
    <img src="../../img/imgredresor/<?php echo $row131['voltag']; ?>_2.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">   
    <img src="../../img/imgredresor/<?php echo $row131['voltag']; ?>_3.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">  
    <img src="../../img/imgredresor/<?php echo $row131['voltag']; ?>_4.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">  
    <img src="../../img/imgredresor/<?php echo $row131['voltag']; ?>_5.jpg" onerror='this.style.display = "none"' onclick="myFunction(this)">  
 ...
 <div class="wow fadeIn articleText" data-wow-duration="2.5s">
  ...
  ?>
   ...
   $titleupakovka = $row131["title"];
        ...
        $query = "SELECT SUM(Column_4) AS Column_5sum FROM `redresorstock` WHERE `Column_3` = '$titleupakovka' AND (`Column_2` = '$linksklad' OR `Column_1` = '$linksklad')";
        $statement = $connect->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll();
...
<?php
   ...
   $query = "SELECT * FROM redresorkrosstablefilter WHERE linkalternator = '$alternatorv'";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
...
<div class="aplicaridiod tablesArt1ProductCrossesonthesite">
 ...
 $link = $row131['link'];
 $query = "SELECT id, imgsrc, link, title, category, voltag, product_status FROM `redresorinitial` WHERE category = '$category' AND product_status = '1' AND link != '$link' AND voltag = '".$voltajj."' ORDER BY id DESC";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 ...
 $totalsum = $rowpsklad['Column_5sum'];
  ...
  <a href="article.php?id='. $row['id'] .'"><li><img class="nodatablock datanotfoundimg1" src="../../img/imgredresor/'.$row['voltag'].'_1.jpg" alt="DATA NOT FOUND">'.$row['link'].''.$finalinf.'</li></a>
...
<div class="tablesArt">
 ...
 <div class="tablesArt1ProductCrosses">
  ...
  <?php
   $alternatorv = $row131['voltag'];
   $query = "SELECT * FROM redresorkrosstable WHERE '$alternatorv' = linkalternator";
   $statement = $connect->prepare($query);
   $statement->execute();
   $result = $statement->fetchAll();
   foreach ($result as $row1311) {
