...
$link = $row131['link'];
 $query = "SELECT id, imgsrc, link, category, voltag, product_status FROM `alternatorinitial` WHERE category = '$category' AND product_status = '1' AND link != '$link' AND voltag = '".$voltajj."' ORDER BY id DESC";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 ...
 $linksklad5 = $row["link"];
 $query = "SELECT SUM(Column_5) AS Column_5sum FROM `stock` WHERE `Column_2` = '$linksklad5' OR `Column_1` = '$linksklad5'";
 ...
 $totalsum = $rowpsklad['Column_5sum'];
   ...
   <a href="article.php?id='. $row['id'] .'"><li><img class="nodatablock datanotfoundimg1" src="'.$row['imgsrc'].'" alt="DATA NOT FOUND">'.$row['link'].''.$finalinf.'</li></a>
